...
import instinct_rl.utils.data_compressor as compressor
from instinct_rl.storage.rollout_files.base import RolloutFileBase
from instinct_rl.utils.collections import namedarraytuple


class RolloutDataset(RolloutFileBase):
    Transition = namedarraytuple(
        "Transition",
        [
            "observation",
    ...
    def __init__(
        ...
        starting_frame_range=[
            0,
            1,
        ],  # if set, the starting timestep will be uniformly chose from this, when each new trajectory is loaded.
    ...
    def read_dataset_directory(self):
        ...
        for data_dir in self.data_dirs:
            if not os.path.exists(data_dir):
                os.mkdir(data_dir)
                print(f"RolloutDataset: {data_dir} not found, created...")
        ...
        print(
            "RolloutDataset: {} trajectories found. {} timesteps in total.".format(
    ...
    def reset_all(self):
        ...
        while not self.read_dataset_directory():
            print(
                "RolloutDataset: trajectory not enough, need {} at least, waiting for 15 minutes...".format(
                    self.keep_latest_n_trajs
                )
            )
            time.sleep(60 * 15)
    ...
    def _refresh_traj_data(self, env_idx):
        ...
        try:
            ...
            raise RuntimeError(
                "RolloutDataset: failed to load trajectory data from {}".format(
    ...
    def _refresh_traj_handler(self, env_idx):
        ...
        while (
            self.get_frame_range(self.traj_file_names[env_idx][self.traj_file_idxs[env_idx]])[0]
            > self.traj_cursors[env_idx]
            or self.get_frame_range(self.traj_file_names[env_idx][self.traj_file_idxs[env_idx]])[1]
            <= self.traj_cursors[env_idx]
        ):
            self.traj_file_idxs[env_idx] += (
                1
                if self.get_frame_range(self.traj_file_names[env_idx][self.traj_file_idxs[env_idx]])[1]
                <= self.traj_cursors[env_idx]
                else -1
            )
    ...
    def get_obs_segment_from_components(self, components=None):
        return self.metadata["obs_segments"]

    def get_buffer(self, num_transitions_per_env=None, env_ids=None):
        if not env_ids is None:
            raise NotImplementedError("RolloutDataset: env_ids is not supported.")
        leading_dims = ([] if num_transitions_per_env is None else [num_transitions_per_env]) + [self.num_envs]
        if (
            not hasattr(self, "_output_transition_buffer")
            or self._output_transition_buffer_leading_dims != leading_dims
        ):
    ...
    def _fill_transition_per_env(self, buffer, env_idx: int):
        ...
        buffer.next_privileged_observation.copy_(
            self.traj_datas[env_idx]["privileged_observations"][traj_cursor_in_file]
        )
