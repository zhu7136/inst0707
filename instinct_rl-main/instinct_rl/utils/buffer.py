...
from instinct_rl.utils.collections import namedarraytuple_like
...
def buffer_from_example(example, leading_dims, share_memory=False):
    ...
    return buffer_type(*(buffer_from_example(v, leading_dims, share_memory) for v in example))
    ...
    if isinstance(buffer, torch.Tensor):
        return_ = torch.cat(
            [
                buffer,
                torch.zeros(
                    buffer.shape[:dim] + (expand_size,) + buffer.shape[dim + 1 :],
                    dtype=buffer.dtype,
        ...
        return_ = np.concatenate(
            [buffer, np.zeros(buffer.shape[:dim] + (expand_size,) + buffer.shape[dim + 1 :], dtype=buffer.dtype)],
            axis=dim,
        )
        if contiguous:
            return_ = np.ascontiguousarray(return_)
    else:
        return_ = type(buffer)(*(buffer_expand(b, expand_size, dim, contiguous) for b in buffer))
    return return_
...
def buffer_swap(buffer, cursor, dim=0, contiguous=False):
    ...
    if isinstance(buffer, torch.Tensor):
        return_ = torch.cat([buffer[cursor:], buffer[:cursor]], dim=dim)
        if contiguous:
            return_ = return_.contiguous()
    elif isinstance(buffer, np.ndarray):
        return_ = np.concatenate([buffer[cursor:], buffer[:cursor]], axis=dim)
        if contiguous:
            return_ = np.ascontiguousarray(return_)
    else:
        return_ = type(buffer)(*(buffer_swap(b, cursor, dim, contiguous) for b in buffer))
    return return_
