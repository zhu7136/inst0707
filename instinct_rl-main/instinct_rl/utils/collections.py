...
def namedarraytuple(typename, field_names, return_namedtuple_cls=False, classname_suffix=False):
    ...
    def __setitem__(self, loc, value):
        ...
        if not (
            ...
            and getattr(value, "_fields", None) == self._fields  # Check for matching structure.
        ):
    ...
    def get_leading_dims(self):
        "Returns the number of dimensions ahead that can be indexed, which means the shape after this dimension will be different between fields. This might be longer than the user expected."
        leading_dims = None
...
def is_namedtuple_class(obj):
    ...
    if obj.mro()[1] is not tuple:
        return False
    if not all(hasattr(obj, attr) for attr in ["_fields", "_asdict", "_make", "_replace"]):
        return False
    return True
...
def namedarraytuple_like(namedtuple_or_class, classname_suffix=False):
    ...
    if is_namedtuple(ntc):
        return namedarraytuple(type(ntc).__name__, ntc._fields, classname_suffix=classname_suffix)
    elif is_namedtuple_class(ntc):
        return namedarraytuple(ntc.__name__, ntc._fields, classname_suffix=classname_suffix)
    elif is_namedarraytuple(ntc):
        return type(ntc)
    elif is_namedarraytuple_class(ntc):
        return ntc
    elif isinstance(ntc, (NamedTupleSchema, NamedTuple, NamedArrayTupleSchema, NamedArrayTuple)):
        return namedarraytuple(ntc._typename, ntc._fields, classname_suffix=classname_suffix)
    else:
        raise TypeError(f"Input must be namedtuple or namedarraytuple instance or class, got {type(ntc)}.")
    ...
    def __init__(self, typename, fields):
        ...
        for field in fields:
            ...
            if field.startswith("_"):
                raise ValueError(f"field names cannot start with an underscore: {field}")
        ...
        self.__dict__["_signature"] = Sig(Param(field, Param.POSITIONAL_OR_KEYWORD) for field in fields)
    ...
    * Attempts to enforce call signatures are included, might not exactly
    ...
    def __new__(cls, typename, fields, values):
        ...
        if len(fields) != len(result):
            raise ValueError(f"Expected {len(fields)} arguments, got {len(result)}")
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            raise AttributeError(f"'{self._typename}' object has no attribute '{name}'")

    def __setattr__(self, name, value):
        """Make the object immutable, like a tuple."""
        raise AttributeError(f"can't set attributes of '{type(self).__name__}' instance")
    ...
    def __repr__(self):
        ...
        return self._typename + "(" + ", ".join(f"{name}={value}" for name, value in zip(self._fields, self)) + ")"
        ...
        try:
            ...
            for j, s in enumerate(self):
                ...
                try:
                    ...
                    raise Exception(f"Occurred in '{self._typename}' at field '{self._fields[j]}'.") from e
...
def NamedArrayTupleSchema_like(example):
    ...
    if isinstance(example, NamedArrayTupleSchema):
        ...
        raise TypeError(
            ...
            " or class, or Named[Array]Tuple[Schema] instance, got "
            f"{type(example)}."
        )
