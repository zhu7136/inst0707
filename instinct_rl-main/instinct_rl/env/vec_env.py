...
class VecEnv(ABC):
    ...
    num_rewards: int
    """Number of rewards. For multi-critic/advantage-mixing, return 1 by default."""
    ...
    @abstractmethod
    def get_obs_format(self) -> dict[str, dict[str, torch.Tensor]]:
        """Return the observation format. Specifying the observation components in each observation group.
        By observation components, it is the same concept as observation term in isaaclab. The dictionary structure
        should be as follows:
        ```
        obs_format = {
            "observation_name": {
                "observation_component_1": observation shape,
                "observation_component_2": observation shape,
                ...
            },
            ...
        }
        ```
        "observation_name" depends on the need of algorithm, for example, "policy", "critic"

        Returns:
            Dictionary containing the observation format.
        """
        raise NotImplementedError

    @abstractmethod
    def get_observations(self) -> tuple[torch.Tensor, dict]:
        """Return the current observations for policy and extras. In extras, `extras["observations"]` should be a dict containing
        all observations defined in `get_obs_format()`.
