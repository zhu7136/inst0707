...
class DemonstrationSaver:
    ...
    def init_traj_handlers(self):
        ...
        if len(os.listdir(self.save_dir)) > 1:
            ...
            with open(osp.join(self.save_dir, "metadata.json")) as f:
    ...
    def init_storage_buffer(self):
        ...
        critic_obs_size = get_subobs_size(obs_format.get("critic", obs_format["policy"]))
        ...
        self.transition_timeouts = torch.zeros(
            self.rollout_storage_length, self.env.num_envs, dtype=torch.bool, device=self.env.device
        )

    def check_stop(self):
        return (self.total_traj_completed >= self.min_episodes) and (self.total_timesteps >= self.min_timesteps)
    ...
    def dump_metadata(self):
        self.metadata["total_timesteps"] = (
            self.total_timesteps.item() if isinstance(self.total_timesteps, np.int64) else self.total_timesteps
        )
        self.metadata["total_trajectories"] = self.total_traj_completed
    ...
    def update_traj_handler(self, env_i, step_slice):
        ...
        if self.success_traj_only:
            if self.rollout_storage.dones[step_slice.stop - 1, env_i] and (
    ...
    def collect_and_save(self, config=None):
        ...
        privileged_obs = extras["observations"].get("critic", None)
    ...
    def __del__(self):
        """In case the process stops accidentally, close the file handlers"""
        for traj_idx in self.traj_idxs:
            traj_dir = osp.join(self.save_dir, f"trajectory_{traj_idx}")
