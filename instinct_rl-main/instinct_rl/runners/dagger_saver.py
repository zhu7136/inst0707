...
from instinct_rl.algorithms.tppo import GET_PROB_FUNC
...
class DaggerSaver(DemonstrationSaver):
    ...
    def build_training_policy(self):
        ...
        with open(osp.join(self.training_policy_logdir, "config.json")) as f:
            config = json.load(f)
    ...
    def load_latest_training_policy(self):
        ...
        if len(models) == 0:
            print(
                "No model found in the training policy logdir. Make sure you don't need to load the training policy or"
                " stop the program."
            )
        ...
        if model_f_iter > self.training_policy_iteration:
            ...
            print(f"Training policy iteration: {self.training_policy_iteration}")
        self.use_teacher_act_mask = torch.rand(self.env.num_envs) < self.teacher_act_prob(
            self.training_policy_iteration
        )
    ...
    def print_log(self):
        ...
        print(f"Sampling saved for training policy iteration: {self.training_policy_iteration}")
